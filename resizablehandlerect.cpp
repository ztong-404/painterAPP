...
/*如果ownerItem的值被选中,就创建四个手柄，否则就移除四个手柄*/
void ResizableHandleRect::drawHandlesIfNecessary()
{
    if(!ownerItem){
        qWarning() << "ResizableHandleRect : No ownerItem set. "
                      "Please call setOwnerItem on your ResizableHandleRect subclass";
                      return;
    }
